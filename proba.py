...
class Server:
    ...
    def middleware(self, user_id):
        # Проверка состояния пользователя перед переходом к новой форме
        if user_id in self.user_states:
            return True  # Пользователь уже заполняет форму
        return False  # Пользователь не заполняет форму
    ...
    def middleware(self, user_id):
        # Проверка состояния пользователя перед переходом к новой форме
        if user_id in self.user_states['dog_forms'] or user_id in self.user_states['user_forms']:
            return True  # Пользователь уже заполняет форму
        return False  # Пользователь не заполняет форму
    ...
    def start(self):
        for event in self.long_poll.listen():
            if event.type == VkEventType.MESSAGE_NEW and event.to_me:
                ...
                FORM_STATES = ['dog_form', 'tree_fall_form', 'rubbish_form', 'car_form', 'light_form', 'traffic_form']# Приводим текст к нижнему регистру
                ...
                if self.user_states[user_id] == 'zayavka_menu':
                    ...
                    if message_text and message_text.lower() == "в главное меню":
                        ...
                        # Применяем мидлварик
                    if self.middleware(event.user_id):
                        self.send_message(event.user_id, "Вы уже заполняете форму. Продолжайте вводить данные.")
                        continue

                    if event.text and event.text.lower() == "отлов собак":
                        if user_id in self.user_states and self.user_states[user_id] in FORM_STATES:
                            self.send_message(user_id,
                                              "Вы уже заполняете форму. Пожалуйста, завершите её, прежде чем начинать новую.")
                            continue
